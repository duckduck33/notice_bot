...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # ✨ 요청: while문이 돌때마다 현재시간같이 출력
            current_time = datetime.now()
            print(f"\n--- [ {current_time.strftime('%Y-%m-%d %H:%M:%S')} ] 루프 시작 ---")

            # ✨ 요청: 1시간마다 작동 확인 알림 추가
            if current_time - last_health_check_time >= timedelta(hours=1):
                ...
                last_health_check_time = current_time # 알림 보낸 시간 갱신
            ...
            if res:
                if res.status_code == 200:
                    ...
                    if notices:
                        ...
                        process_notice_by_id(notice_id) # is_listing_notice는 process_notice_by_id 내부에서 처리
                    else:

...
LAST_NOTICE_WARN_PATH = os.path.join(os.path.dirname(__file__), 'last_notice_warn.json')
...
default_last_notice = {
    "id": "9999",
    "title": "칼데라(ERA) 신규 거래지원 안내 (KRW, BTC, USDT 마켓) (매도 최저가 기준 가격 안내)",
    "listed_at": "",
    "first_listed_at": "",
    "assets": [
        {"asset": "칼데라", "trade_time": "2025-07-18 01:00 KST"}
    ]
}
default_last_notice_warn = {
    "id": "8888",
...
if not os.path.exists(LAST_NOTICE_WARN_PATH):
    with open(LAST_NOTICE_WARN_PATH, 'w', encoding='utf-8') as f:
        json.dump(default_last_notice_warn, f, ensure_ascii=False, indent=2)
...
def extract_asset_from_warn_title(title):
    match = re.match(r'([^(]+)\(', title)
    if match:
        return match.group(1).strip()
...
def process_notice_by_id(notice_id):
    ...
    # 상장 공지 처리
    if is_listing_notice(title, config):
        last_notice = load_json(LAST_NOTICE_PATH)
        if (
            last_notice and
            str(notice_id) == str(last_notice.get("id")) and
            listed_at == last_notice.get("listed_at")
        ):
            print("\n신규 상장 공지 없음 (기존 데이터와 동일)")
            return

        pattern_type = 1 if listed_at == first_listed_at else 2
    ...
    # 유의 공지 처리
    if is_warn_notice(title, config):
        last_notice_warn = load_json(LAST_NOTICE_WARN_PATH)
        asset = extract_asset_from_warn_title(title)
        if last_notice_warn and str(notice_id) == str(last_notice_warn.get("id")):
...
def latest_notice():
    try:
        with open(LAST_NOTICE_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data
    except Exception as e:
        return {"error": str(e)}

@app.get("/latest_notice_warn")
def latest_notice_warn():
    try:
        with open(LAST_NOTICE_WARN_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data
    except Exception as e:
        return {"error": str(e)}
    ...
    while True:
        try:
            ...
            if current_time - last_health_check_time >= timedelta(hours=1):
                send_telegram_message("✅ <b>상장/유의 감시봇이 정상 작동 중입니다.</b>", chat_id=ADMIN_CHAT_ID)
